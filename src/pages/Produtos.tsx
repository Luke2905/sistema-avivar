...
// Interface alinhada com o retorno do MySQL (Maiúsculas)
interface Produto {
  ID_PRODUTO: number;
  SKU_PRODUTO: string;
  NOME_PRODUTO: string;
...
export default function Produtos() {
  ...
  // Estados para controlar o Modal de Ficha Técnica
  ...
  async function handleSalvar(e: FormEvent) {
    ...
    try {
      await api.post('/produtos', { 
        ...
        id_categoria: 1 // Fixo por enquanto (Fase futura: Select de categorias)
      });
      
      showToast('Produto cadastrado!');
      setSku(''); setNome(''); setPreco(''); // Limpa o form
  ...
  const handleExcluir = async (id: number, nome: string) => {
    ...
    if (result.isConfirmed) {
      try {
        // Supondo que exista a rota DELETE /produtos/:id. 
        // Se não existir, você precisará criar no backend ou o botão dará erro.
